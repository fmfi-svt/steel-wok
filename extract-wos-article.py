...
async def extract_text_from_selector(selector: str, page: Page,
                                     default_value: str = ''):
...
async def main_extract(url: str,
                       debug: bool = False,
                       proxy_server: str = ''):
    ...
    async with async_playwright() as p:
        ...
        for selector in SELECTORS.keys():
            text = await extract_text_from_selector(SELECTORS[selector],
                                                    page)
        ...
        for row in journal_content_rows:
            ...
            value = ', '.join([await span.inner_text()
                               for span in value_selector])
